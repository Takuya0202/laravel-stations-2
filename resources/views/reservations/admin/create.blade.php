    ...
    <form action="{{route('admin.reservation.store')}}" method="post">
        ...
        <select name="sheet_id" id="">
            @foreach ($sheets as $sheet)
                <option value="{{$sheet->id}}" @selected($sheet->id == old('sheet_id'))>{{$sheet->row}}~{{$sheet->column}}</option>
            @endforeach
        </select>
        <br>
        ...
        <select name="schedule_id" id="">
            @foreach ($schedules as $sc)
                <option value="{{$sc->id}}" @selected($sc->id == old('schedule_id'))>{{$sc->start_time->format('Y-m-d')}}~{{$sc->end_time->format('Y-m-d')}}</option>
            @endforeach
        </select>
        <br>
